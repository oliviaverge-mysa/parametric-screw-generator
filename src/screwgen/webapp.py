...
app = FastAPI(title="Screw Generator Chat")
...
def _write_engineering_drawing_svg(spec, output_path: Path) -> None:
  ...
  <text x="40" y="70" class="small">Type: {esc(spec.head.type)} | Drive: {esc(drive)} | Generated by Screw Generator</text>
...
def _write_engineering_drawing_pdf(
    ...
    c.drawString(ix + 44, iy + ih - 36, f"Screw: {screw_label}")
    ...
    # Head-height dimension intentionally closer to the screw than full-length.
    _dim_h(
        side_x,
        x_body0,
        side_y - head_r - 14,
