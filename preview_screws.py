...
Build order:
1) make_head
2) apply_drive_to_head
3) make_shaft
4) union shaft to driven head
"""
...
SHAFT_VARIANTS = {
    "A": ShaftParams(d_minor=3.0, L=20.0, tip_len=3.0),
    "B": ShaftParams(d_minor=4.0, L=35.0, tip_len=4.0),
}
...
def build_gallery_solids() -> list[cq.Workplane]:
    """Build translated screws for gallery layout."""
    ...
    for h_idx, head_type in enumerate(HEAD_TYPES):
        ...
        for d_idx, (dtype, dsize) in enumerate(DRIVE_TYPES):
            ...
            for s_idx, (_, sp) in enumerate(SHAFT_VARIANTS.items()):
                if driven is None:
                    # avoid recutting same head for each shaft variant
                ...
                # Grid: X=head groups with sub-column for shaft variant, Y=drive.
