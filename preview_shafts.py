...
Exports STEP+STL for:
  - Shaft variants.
  - Head+shaft combinations for all head types.
  - Combined library STEP with all combinations laid out in a grid.
  - Sectioned library STEP for quick interface/tip inspection.
"""
...
# Layout spacing for screw_library.step
...
def _section_x_negative(solid: cq.Workplane) -> cq.Workplane:
    """Keep X<=0 half for section visualization."""
...
def build_screw_library_solids() -> list[cq.Workplane]:
    """Build all head+shaft combo solids translated into a review grid."""
    ...
    for h_idx, head_name in enumerate(_HEAD_ORDER):
        ...
        for sp in variants:
            ...
            # X grouped by head type, then by length.
            x = h_idx * _SPACING_HEAD_X + L_idx * _SPACING_L_X
            # Y grouped by d_minor, with tip_len as a secondary offset row.
...
def export_screw_library(output_dir: Path = OUTPUT_DIR) -> tuple[Path, Path, int]:
    """Export screw_library.step and screw_library_section.step.

    Returns (library_path, section_path, solid_count).
    """
...
def main() -> None:
    ...
    for head_name in _HEAD_ORDER:
        ...
        for sp in variants:
            ...
            combo_base = (
                f"screw_{head_name}__dminor{_fmt(sp.d_minor)}"
                f"_L{_fmt(sp.L)}_tip{_fmt(sp.tip_len)}"
            )
    ...
    print("\n== Combined library exports ==")
    library_path, section_path, count = export_screw_library(OUTPUT_DIR)
    print(f"  screw_library.step -> {library_path}")
